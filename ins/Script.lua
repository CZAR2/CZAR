...
if not msg.malk then return "✠*︙*هذا الامر يخص {المطور} فقط  \n✠" end
...
if not msg.SudoBase then return"✠*︙*هذا الامر يخص {المطور الاساسي} فقط  \n✠" end
local setch = redis:get(CZAR..":setch")
if setch then
return "✠︙اهلا عزيزي المطور \n✠︙الاشتراك الاجباري للقناة : @["..setch.."]\n✓"
else
...
if redis:sismember(CZAR..'Gp2:'..msg.chat_id_,'Welcomeon') then
if msg.content_.ID == 'MessageChatAddMembers' or msg.content_.ID == 'MessageChatJoinByLink' then 
function WelcomeByAddUser(BlaCk,Diamond)
local function setlinkgp(td,kara)
function gps(arg,data)
links = redis:get(CZAR..'linkGroup'..msg.chat_id_) or 'not'
if redis:get(CZAR..'Text:Welcome:'..msg.chat_id_) then
txtt = redis:get(CZAR..'Text:Welcome:'..msg.chat_id_)
else
txtt = '❂┓ اهلآ عزيزي '..(Diamond.first_name_ or '---')..'\n👥┫ المجموعه  '..(data.title_ or '---')..' \n🤫┫ احترام الادمنيه\n❂┫ ممنوع طلب الرتب\n❂┫ ممنوع السب والكفران \n❂┫ ممنوع التكلم بالطائفيه\n❂┫ الاعضاء '..kara.member_count_..' عضو\n❂┫ الادمنيه '..kara.administrator_count_ ..' \n❂┫ وقت الانضمام :【*'..os.date("%H:%M:%S")..'*】\n❂┛ تاريخ الانضمام :【*'..os.date("%Y/%m/%d")..'*】\n'
end
time = ''..os.date("%H:%M:%S")..''
date = ''..os.date("%Y/%m/%d")..''
local txtt = txtt:gsub('FIRSTNAME',Diamond.first_name_)
local txtt = txtt:gsub('GROUP',(data.title_ or 'المجموعه'))
local txtt = txtt:gsub('LINK',(links or 'ارابط غير موجود'))
local txtt = txtt:gsub('ADMIN',(kara.administrator_count_ or 'البوت ليس مشرف'))
local txtt = txtt:gsub('MEMBER',(kara.member_count_ or 'البوت ليس مشرف'))
local txtt = txtt:gsub('USERNAME','@'..Flter_Markdown(Diamond.username_ or 'nil'))
local txtt = txtt:gsub('TIME',time)
local txtt = txtt:gsub('DATE',date)
sendMsg(msg.chat_id_,msg.id_,txtt)
end
...
if redis:get(CZAR..'wel'..msg.chat_id_) and msg.text then ----- استقبال الترحيب
redis:set(CZAR..'Text:Welcome:'..msg.chat_id_,msg.text)
sendMsg(msg.chat_id_,msg.id_,'تم وضع الترحيب على '..msg.text..'')
redis:del(CZAR..'wel'..msg.chat_id_,60,true)
end

if Black and (Black:match('ضع كليشه الترحيب')) then
sendMsg(msg.chat_id_,msg.id_,'ارسل لي الترحيب')
redis:setex(CZAR..'wel'..msg.chat_id_,60,true)
end

if Black == 'welcome on' or Black == 'تفعيل الترحيب' and is_JoinChannel(msg) then
if redis:sismember(CZAR..'Gp2:'..msg.chat_id_,'Welcomeon') then
sendMsg(msg.chat_id_,msg.id_,'تم تفعيل الترحيب سابقا')
else
sendMsg(msg.chat_id_,msg.id_,'تم تفعيل الترحيب')
redis:sadd(CZAR..'Gp2:'..msg.chat_id_,'Welcomeon')
end
end
if Black == 'welcome off' or Black == 'تعطيل الترحيب' and is_JoinChannel(msg) then
if redis:sismember(CZAR..'Gp2:'..msg.chat_id_,'Welcomeon') then
redis:srem(CZAR..'Gp2:'..msg.chat_id_,'Welcomeon')
